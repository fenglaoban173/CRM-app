...
function DateRangePicker({ value, onChange }) {
  ...
  const [cursorMonth, setCursorMonth] = useState(dayjs(value.start).month() + 1) // 1-based
  ...
  const handleApply = () => {
    ...
    else if (start) onChange({ start, end: start })  // 只选了开始 → 视为单日
    setOpen(false)
  }
  ...
  const isInRange = (dateStr) => start && end && dateStr >= start && dateStr <= end
  const isStart   = (dateStr) => dateStr === start
  const isEnd     = (dateStr) => dateStr === end
        ...
        <div className="absolute top-full left-0 mt-1 z-50 bg-white rounded-xl shadow-xl border border-ink-100 w-[280px] overflow-hidden">
          ...
          <div className="flex items-center justify-between px-3 py-2">
            <button onClick={() => shiftMonth(-1)}
              className="w-7 h-7 rounded-full flex items-center justify-center tap hover:bg-ink-50"
              aria-label="上个月">
          ...
          <div className="grid grid-cols-7 gap-1 px-3 pb-3">
            ...
            {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(d => {
              ...
              const isStartEdge = isStart(dateStr)
              const isEndEdge = isEnd(dateStr)
          ...
          <div className="flex border-t border-ink-100 px-3 py-2.5 bg-white">
            <button onClick={handleApply}
              className="w-full h-9 bg-brand text-white rounded-full text-[13px] tap active:opacity-90">
              确定
            </button>
          </div>
        </div>
      ...
      <div className="w-full bg-white rounded-t-2xl h-[80vh] flex flex-col relative" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex flex-1 min-h-0">
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {/* 统计日期：日期区间 picker（与行 1 一致）*/}
            {activeField?.kind === 'periodPicker' && (
              <div className="space-y-3">
                <DateRangePicker value={values.period} onChange={v => set('period', v)}/>
                <div className="text-[11px] text-ink-400 tabular-nums">当前选择：{values.period.start} 至 {values.period.end}</div>
              </div>
            )}
            {/* 媒体平台：多选 chips */}
...
// ============ 主页面 ============
export default function MediaDailyReportPage({ node }) {
  ...
  const handleAdvClose = () => {
    ...
    if (adv.period && (adv.period.start !== period.start || adv.period.end !== period.end)) {
      setPeriod(adv.period)
    }
    setAdvOpen(false)
  }
  ...
  const activeAdvCount =
    ...
    (adv.period.start !== period.start || adv.period.end !== period.end ? 1 : 0)
