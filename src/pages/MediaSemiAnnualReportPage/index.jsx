...
// ============ 常量 ============
const TODAY = dayjs()
// 上半年（H1: 1-6月）或下半年（H2: 7-12月）
const _lastH = TODAY.month() < 6 ? 2 : 1
const _lastHYear = _lastH === 2 ? TODAY.year() - 1 : TODAY.year()
const DEFAULT_PERIOD = `${_lastHYear}-H${_lastH}`
...
// ============ 年-半年 日期组件 ============
function HalfYearPicker({ value, onChange }) {
  const [open, setOpen] = useState(false)
  // value 格式：YYYY-Hn
  const m = /^(.*)-H([1-2])$/.exec(value || '')
  const valueYear = m ? Number(m[1]) : dayjs().year()
  const valueH = m ? Number(m[2]) : 1
        ...
        <div className="absolute top-full left-0 mt-1 z-50 bg-white rounded-xl shadow-xl border border-ink-100 p-3 w-[268px]">
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              const active = valueYear === year && valueH === hh
              return (
                <button key={hh} onClick={() => handleSelect(hh)}
                  className={`h-10 rounded-lg text-[12px] tap tabular-nums ${
  ...
  return (
    <div className="mx-2 mt-2 card">
      <div className="flex items-center gap-2 px-2.5 py-2">
        ...
        <HalfYearPicker value={period} onChange={setPeriod}/>
      ...
      <div className="w-full bg-white rounded-t-2xl h-[80vh] flex flex-col relative" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex flex-1 min-h-0">
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {/* 统计日期：半年 picker（与行 1 一致）*/}
            {activeField?.kind === 'periodPicker' && (
              <div className="space-y-3">
                <HalfYearPicker value={values.period} onChange={v => set('period', v)}/>
...
// ============ 主页面 ============
export default function MediaSemiAnnualReportPage({ node }) {
