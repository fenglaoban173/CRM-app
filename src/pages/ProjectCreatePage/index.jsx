...
export default function ProjectCreatePage() {
  ...
  const togglePolicyCollapse = (idx) => setCollapsed(prev => {
      ...
      <div className="space-y-3 px-3 pt-2">
        {policies.map((p, idx) => (
          <PolicyCard
            ...
            onToggle={() => togglePolicyCollapse(idx)}
...
// ============ 政策卡 ============
function PolicyCard({ index, policy, platforms, initialModes, serviceOrderTypes, payTypes, customerTypes, owners, canDelete, collapsed, onToggle, onChange, onRemove }) {
  // 折叠态摘要：平台 / 合作模式 / 客户类别（最具辨识度的三个字段）
  const summaryText = [policy.platform, policy.initialMode, policy.customerType]
    .filter(Boolean)
    .join(' · ') || '未填写'

  return (
    <div className="card overflow-hidden">
      {/* 卡标题 + 摘要 + 删除 + 折叠图标（整行可点击折叠）*/}
      <div
        onClick={onToggle}
        className="px-4 py-2.5 flex items-center justify-between cursor-pointer tap active:bg-ink-50"
      >
        <div className="flex items-center gap-2 min-w-0 flex-1">
          <span className="text-[13px] font-medium text-ink-900 shrink-0">政策 {index + 1}</span>
          {collapsed && (
        ...
        <div className="flex items-center gap-1 shrink-0">
          {canDelete && (
            <button
              ...
              className="h-6 px-2 text-danger text-[12px] flex items-center gap-1 tap active:opacity-80"
            >
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none">
