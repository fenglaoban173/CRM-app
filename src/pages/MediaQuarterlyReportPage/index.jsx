...
import {
  PieChart, Pie, Cell, ResponsiveContainer, BarChart, Bar, XAxis, YAxis, Tooltip, CartesianGrid, Legend, LabelList,
} from 'recharts'

// ============ 常量 ============
const TODAY = dayjs()
// 上一季度（如 2026-08 → 2026-Q2）
const _lastQ = TODAY.month() < 3 ? 4 : Math.floor(TODAY.month() / 3)
const _lastQYear = _lastQ === 4 && TODAY.month() < 3 ? TODAY.year() - 1 : TODAY.year()
const DEFAULT_PERIOD = `${_lastQYear}-Q${_lastQ}`
  ...
  return (
    <div className="mx-2 mt-2 card">
      <div className="flex items-center gap-2 px-2.5 py-2">
        ...
        <QuarterYearPicker value={period} onChange={setPeriod}/>
...
function IndustryStackChart({ industryStack }) {
  const [activeMedia, setActiveMedia] = useState(null)  // null | '头条-AD' | '头条-本地推' | '头条-千川'
  if (!industryStack || industryStack.length === 0) return null
  // 颜色：头条-AD 橙 / 头条-本地推 绿 / 头条-千川 蓝
  ...
  const rightText = activeMedia
    ? `已选：${activeMedia}　总计：${focusedTotal.toFixed(2)} 万　平均值：${focusedAvg.toFixed(2)} 万`
    : `总计：${focusedTotal.toFixed(2)} 万　平均值：${focusedAvg.toFixed(2)} 万`
  ...
  return (
    <div className="mx-3 mt-3 card overflow-hidden">
      <div className="px-4 py-3 flex items-center justify-between border-b border-ink-100">
        <GroupTitle right={rightText}>消耗总额</GroupTitle>
      </div>
    ...
    <div className="mx-3 mt-3 card overflow-hidden">
      ...
      <div className="px-2 py-3">
        <ResponsiveContainer width="100%" height={180}>
          <BarChart data={bars} margin={{ top: 6, right: 8, bottom: 0, left: -20 }} barCategoryGap="22%">
            <CartesianGrid strokeDasharray="3 3" stroke="#F0F2F5" vertical={false}/>
            <XAxis dataKey="day" tick={{ fontSize: 9, fill: '#9CA3AF' }} interval={3} axisLine={false} tickLine={false}/>
            <YAxis tick={{ fontSize: 9, fill: '#9CA3AF' }} axisLine={false} tickLine={false}/>
            <Tooltip
            ...
            />
            <Bar dataKey="value" fill="#2D7FF9" radius={[2, 2, 0, 0]} maxBarSize={32}>
      ...
      <div className="w-full bg-white rounded-t-2xl h-[80vh] flex flex-col relative" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex flex-1 min-h-0">
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {/* 统计日期：季度 picker（与行 1 一致）*/}
            {activeField?.kind === 'periodPicker' && (
              <div className="space-y-3">
                <QuarterYearPicker value={values.period} onChange={v => set('period', v)}/>
...
// ============ 主页面 ============
export default function MediaQuarterlyReportPage({ node }) {
  ...
  return (
    <div className="bg-ink-50 pb-4 min-h-full">
      ...
      {/* 行业 Top10 三媒体堆叠（季报专用）—— 在消耗总额柱状图之上 */}
