...
// ============ 常量 ============
const TODAY = dayjs()
const DEFAULT_PERIOD = String(TODAY.year() - 1)
...
// ============ 年-选择 日期组件 ============
function YearOnlyPicker({ value, onChange }) {
  const [open, setOpen] = useState(false)
  const [year, setYear] = useState(Number(value) || dayjs().year())
  ...
  useEffect(() => { if (!open) setYear(Number(value) || dayjs().year()) }, [open]) // eslint-disable-line react-hooks/exhaustive-deps

  const handleSelect = () => {
    onChange(String(year))
    setOpen(false)
  }
  // 显示最近 9 年（去年 + 今年 + 之前 7 年）
  const curYear = dayjs().year()
  const years = []
  for (let y = curYear - 1; y >= curYear - 8; y--) years.push(y)
      ...
      {open && (
        <div className="absolute top-full left-0 mt-1 z-50 bg-white rounded-xl shadow-xl border border-ink-100 p-3 w-[268px]">
          ...
          <div className="grid grid-cols-3 gap-2">
            {years.map(y => {
              const active = Number(value) === y
              return (
                <button key={y} onClick={() => { setYear(y); onChange(String(y)); setOpen(false) }}
  ...
  return (
    <div className="mx-2 mt-2 card">
      <div className="flex items-center gap-2 px-2.5 py-2">
        ...
        <YearOnlyPicker value={period} onChange={setPeriod}/>
      ...
      <div className="w-full bg-white rounded-t-2xl h-[80vh] flex flex-col relative" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex flex-1 min-h-0">
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {/* 统计日期：年 picker（与行 1 一致）*/}
            {activeField?.kind === 'periodPicker' && (
              <div className="space-y-3">
                <YearOnlyPicker value={values.period} onChange={v => set('period', v)}/>
...
// ============ 主页面 ============
export default function MediaYearlyReportPage({ node }) {
